...
space = hp.choice('main_choice',[{
    ...
    'learning_rate'       : hp.choice('l_rate',[0.3,0.2,0.1,0.01,.0001]),
    ...
    'positive_reward'     : hp.choice('p_reward',[0.1,0.05,0.01,0.001]),
    ...
    'epsilon_decay'       : hp.choice('eps_decay',[0.993,0.996,0.9997,0.9998,0.9999,1.0]),
    'epsilon_min'         : hp.choice('eps_min',[0.02,0.01,0.005,0.001]),
    },
    {
    'activation_function' : hp.choice('a_function2',['tanh','sigmoid','linear_rectifier','linear']),
    'num_hidden'          : hp.choice('n_hidden2',[8,16,32,64,96,128,256]),
    'learning_rate'       : hp.choice('l_rate2',[0.3,0.2,0.1,0.01,.0001]),
    'learning_rate_decay' : hp.choice('l_rate_decay2',[0.9992,0.9995,0.9997,0.9998,0.9999,1.0]),
    'learning_rate_min'   : hp.choice('l_rate_min2',[0.00001,0.0001,0.001]),
    'positive_reward'     : hp.choice('p_reward2',[0.1,0.05,0.01,.001]),
...
trials = MongoTrials(mongodb_machine + 'cartpole_nn_tests_bugfixed/jobs',exp_key='cartpole_hyperopt_nn_initialtest3')
