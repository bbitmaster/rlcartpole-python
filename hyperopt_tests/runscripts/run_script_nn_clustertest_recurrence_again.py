...
space = {
    ...
    'action_dupe_count'         : hp.choice('a_dupe',[1,2,5,10,20,50]),
    'state_dupe_count'          : hp.choice('s_dupe',[1,2,5,10,20,50]),
    ...
    'create_pomdp'               : hp.choice('pomdp_type',[1,2,3,4]),
    'do_recurrence'              : hp.choice('d_recurrence',[True,False]),
    'use_full_output'            : hp.choice('f_output',[True,False]),
...
def objective(space):
    ...
    params['results_dir'] = os.path.abspath(os.path.join(cwd,'../newton_results_more/')) + '/'

    #set any parameteers here to override the parameters in the params file
    params['simname'] = 'cartpole_hyperopt_nn_clustertest_recurrence_pomdp_more'
...
trials = MongoTrials(mongodb_machine + 'cartpole_clustertest_pomdp_more/jobs',exp_key='cartpole_hyperopt_nn_clustertest')
